...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    // Now send the detected text to GPT API via your /api/refine endpoint
    const gptResponse = await fetch("/api/refine", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
      body: JSON.stringify({ prompt: detectedText, initialInput: "" }), // adjust initialInput as needed
    });
